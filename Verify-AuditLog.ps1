...
foreach ($tenant in $targetTenants) {
    ...
    $info = $found[$tenant]
    if (-not $info) {
        Write-Host "[$i/19] $tenant - NOT FOUND IN EXCEL" -ForegroundColor Red
    ...
    $auditResult = powershell.exe -NoProfile -ExecutionPolicy Bypass -Command {
        ...
        if ($enabled -eq $true) { Write-Output "ENABLED" } else { Write-Output "NOT_ENABLED" }
    } -Args $info.User, $info.Pass, $info.Domain 2>&1

    $output = ($auditResult | Out-String).Trim()

    if ($output -match "ENABLED" -and $output -notmatch "NOT_ENABLED") {
        Write-Host " ENABLED" -ForegroundColor Green
        $results += [PSCustomObject]@{Num=$i; Tenant=$tenant; Status="ENABLED"}
    } elseif ($output -match "NOT_ENABLED") {
        Write-Host " NOT ENABLED" -ForegroundColor Red
        $results += [PSCustomObject]@{Num=$i; Tenant=$tenant; Status="NOT_ENABLED"}
    } else {
        Write-Host " ERROR" -ForegroundColor Yellow
        $results += [PSCustomObject]@{Num=$i; Tenant=$tenant; Status="ERROR: $output"}
    }
}
...
$notEnabled = ($results | Where-Object {$_.Status -eq 'NOT_ENABLED'}).Count
$errors = ($results | Where-Object {$_.Status -like 'ERROR*'}).Count
Write-Host "Enabled: $enabled | Not Enabled: $notEnabled | Errors: $errors"
