...
SCREENSHOTS_DIR = rf"C:\certs\screenshots_{SPN_SHEET}"
REPORT_FILE = rf"C:\certs\dspm_report_{SPN_SHEET}.md"
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    screenshot_path = os.path.join(SCREENSHOTS_DIR, f"{safe_name}_{result['status']}.png")
...
def save_results(all_results):
    ...
    fieldnames = ["username", "tenant_id", "status", "job_title", "items_found", "job_date", "checked_at", "screenshot"]
...
def generate_report(all_results):
    """Generate a markdown report with screenshots for each account."""
    all_vals = sorted(all_results.values(), key=lambda r: r.get("status", ""))
    with open(REPORT_FILE, "w", encoding="utf-8") as f:
        f.write(f"# DSPM Status Report - {SPN_SHEET}\n\n")
        ...
        for r in all_vals:
            ...
            if screenshot and os.path.exists(screenshot):
                rel_path = f"screenshots_{SPN_SHEET}/{os.path.basename(screenshot)}"
                f.write(f"\n![{user}]({rel_path})\n")
            else:
